...
instance Ord Country where
  ...
  (<=) a b
    | (a == Finland || a == Norway) && (b == Norway || b == Switzerland) = True
    | a == Switzerland && (b == Norway || b == Finland) = False
    | a == Norway && b == Switzerland = True
    | a == Norway && b == Finland = False
    | a == b = True
    | otherwise = False
  -- min = todo -- and me?
  -- max = todo -- and me?
...
instance Eq a => Eq (List a) where
  ...
  (==) (LNode firstNode firstList) (LNode secondNode secondList) =
    (firstNode == secondNode) && (firstList == secondList)
...
instance Price a => Price [a] where
  price itemList = sum $ map price itemList
...
simplify (RationalNumber num denom) = RationalNumber (num `div` gcdRes) (denom `div` gcdRes) where
...
instance Num RationalNumber where
  ...
  abs (RationalNumber n d) = RationalNumber (pos n) (pos d) where
    pos x = if x < 0 then negate x else x
...
class Cycle a where
  ...
  stepMany :: Int -> a -> a
  stepMany 0 val = val
  stepMany i val = stepMany (i-1) (step val)
