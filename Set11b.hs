...
appendAll stringRef strings = do
    curr <- readIORef stringRef
    writeIORef stringRef (concat (curr:strings))
...
get1Idxs :: [String] -> [Int] -> [String]
get1Idxs _ [] = []
get1Idxs [] _ = []
get1Idxs lines (num':nums') = let currLine = lines !! (num' - 1) in -- 1-indexed
    currLine:get1Idxs lines nums'

hSelectLines :: Handle -> [Int] -> IO [String]
hSelectLines _ [] = return []
hSelectLines h nums = do
    allLines <- hFetchLines h
    return $ get1Idxs allLines nums

-- This one only works if provided line numbers are in order
hSelectOrderedLines :: Handle -> [Int] -> IO [String]
hSelectOrderedLines _ [] = return []
hSelectOrderedLines h (num:nums) = go 1 where
    go lineNum = do
        eof <- hIsEOF h
        nextLine <- if eof || lineNum /= num then return [] else hGetLine h
        nextRecurse <- hSelectOrderedLines h nums
        return (nextLine:nextRecurse)
...
interact' f state = do
    command <- getLine
    let (resultBool, message, newState) = f (command, state)
    putStrLn message
    if resultBool then interact' f newState else return newState
