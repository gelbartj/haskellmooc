...
countHelper curr running = running + maybe 1 (const 0) curr
...
slHelper currSum (runningSum, runningLength) = (currSum + runningSum, runningLength + 1)
...
concatHelper :: [a] -> [a] -> [a]
concatHelper curr ongoing = curr ++ ongoing
...
largestHelper curr ongoing = if null ongoing then [curr] else 
    case compare curr $ myMaximum ongoing of
        LT -> ongoing
        GT -> [curr]
        EQ -> curr : ongoing
