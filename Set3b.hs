...
buildList start count end = if count == 0 then [end] else
...
sums :: Int -> [Int]
sums 0 = []
sums i = 1 : if i == 1 then [] else sums' 1 2 where 
    sums' prev curr = prev + curr : if curr == i then [] else 
        sums' (prev + curr) (curr + 1)
...
mylast :: a -> [a] -> a
mylast def [] = def
mylast def (first:second:rest) = mylast def (second:rest)
mylast _ (first:_) = first
...
indexDefault (first:rest) i def = if i < 0 then def else
    index' (first:rest) i 0
    where 
        index' [] _ _ = def
        index' (first':rest') i' counter = if counter == i' then first' else index' rest' i' (counter + 1)
...
sorted :: [Int] -> Bool
sorted [] = True
sorted (x:xs) = xs == [] || sorted' x xs where
    sorted' _ [] = True
    sorted' prev (next:rest) = next >= prev && sorted' next rest
...
sumsOf :: [Int] -> [Int]
sumsOf [] = []
sumsOf (x:xs) = x : sumsOf' x xs where
    sumsOf' _ [] = []
    sumsOf' prev (next:rest) = (prev + next) : sumsOf' (prev + next) rest
...
mymaximum bigger initial vals = max' initial vals where
        max' winningVal [] = winningVal
        max' winningVal (val:vals) = max' newWinner vals where
            newWinner = if winningVal `bigger` val then winningVal else val
