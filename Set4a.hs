...
longest :: Ord a => [[a]] -> [a]
longest [] = []
longest (x:xs) = foldr getLonger x xs where
    getLonger a b = case comparing length a b of
        LT -> b
        GT -> a
        EQ -> case compare (head a) (head b) of
            LT -> a
            GT -> b
            EQ -> def
        where def = a   -- arbitrary
...
incrementKey keyCheck = map updateIfKey where
    updateIfKey (key, val) = (key, newVal) where
        newVal = if key == keyCheck then val + 1 else val
...
winner scores player1 player2 = compareScores (getScore player1) (getScore player2) where
    getScore p = Map.findWithDefault 0 p scores
    compareScores first second = case compare first second of
      LT -> player2
      GT -> player1
      EQ -> player1
...
freqs xs = foldr updateCounts startMap xs where
    alterFunc (Just i) = Just (i + 1)
    alterFunc Nothing = Nothing
    updateCounts k map = Map.alter alterFunc k map
    startMap = Map.fromList [(val, 0) | val <- xs]

-- accumArray is an interesting option here too
...
transfer from to amount bank = if isValid then newBank else bank where
    fromBalance = Map.findWithDefault 0 from bank
    toBalance = Map.findWithDefault 0 to bank
    isValid = Map.member from bank && Map.member to bank && amount >= 0 && 
        fromBalance >= amount
    newBank = Map.insert from (fromBalance - amount) (Map.insert to (toBalance + amount) bank)
------------------------------------------------------------------------------
...
maxIndex arr = foldr largestIdx firstVal indexList where
    indexList = Data.Array.indices arr
    firstVal = head indexList
    largestIdx currIdx runningIdx = 
        if (arr ! currIdx) > (arr ! runningIdx) then currIdx else runningIdx
