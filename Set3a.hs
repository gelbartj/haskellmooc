...
firstHalf s = take (length s `div` 2 + (if odd (length s) then 1 else 0)) s
...
joinToLength len strings =
  [ combo | i <- strings, j <- strings, let combo = i ++ j, length combo == len
  ]
...
sumRights eithers = sum $ map (fromRight 0) eithers

-- or either (const 0) id
...
multiCompose (f : fs) arg = (f . go fs) arg
  where
    go [] = id
    go (f' : fs') = f' . go fs'
...
multiApp f gs arg = f [g arg | g <- gs]
...
parseCommands :: (Show t, Num t) => [[Char]] -> t -> t -> [String]
parseCommands [] _ _ = []
parseCommands commands x y =
  if init currObj == "print"
      then show (if last currObj == 'X' then x else y) : parseRest
      else parseRest
  where
    currObj = head commands
    newX = case currObj of
      "right" -> x + 1
      "left" -> x - 1
      _ -> x
    newY = case currObj of
      "up" -> y + 1
      "down" -> y - 1
      _ -> y
    parseRest = parseCommands (tail commands) newX newY