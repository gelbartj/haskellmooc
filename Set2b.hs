...
oddFactorial n
 ...
 | otherwise = n * oddFactorial (max 0 (n - 2))
...
leftpad inputString desiredLength
 | length inputString >= desiredLength = inputString
 | otherwise = leftpad (" " ++ inputString) desiredLength
...
countdown i = "Ready! " ++ countdown' i
 where countdown' n = case n of 0 -> "Liftoff!"
                                _ -> show n ++ "... " ++ countdown' (n - 1)
...
smallestDivisor n = sd n n
 where sd i smallestSoFar
        | i == 1 = smallestSoFar
        | n `mod` i == 0 = sd (i - 1) i
        | otherwise = sd (i - 1) smallestSoFar
