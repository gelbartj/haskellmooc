...
up (MkPosition xVal yVal) = MkPosition xVal (yVal + 1)

-- right increases the x value of a position by one
right :: Position -> Position
right (MkPosition xVal yVal) = MkPosition (xVal + 1) yVal
...
tick :: UpDown -> UpDown
tick (UpCounter n) = UpCounter (n + 1)
tick (DownCounter n) = DownCounter (n - 1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (UpCounter n) = DownCounter n
toggle (DownCounter n) = UpCounter n
...
rgb Green = [0,1,0]
rgb Blue = [0,0,1]
rgb (Mix c1 c2) = zipWith (curry average') (rgb c1) (rgb c2) where
  average' vals = uncurry (+) vals / 2
...
fromBin :: Bin -> Int
fromBin = fromBin' 0 where
  fromBin' power bin = case bin of
    End -> 0
    I b -> (2 ^ power) + fromBin' (power + 1) b
    O b -> fromBin' (power + 1) b

toBin :: Int -> Bin
toBin 0 = O End
toBin 1 = I End
toBin n = if modResult == 1 then I $ toBin newVal else O $ toBin newVal where
  modResult = n `mod` 2
  newVal = n `div` 2
