...
capitalize (MakeName []) = MakeName []
capitalize (MakeName ss) = MakeName capitalized where
  capitalized = unwords $ map (\x -> toUpper (head x):tail x) (words ss)

toFull :: Name First -> Name Last -> Name Full
toFull f n = MakeName (fromName f ++ " " ++ fromName n)
