...
outputInInput input output = forAll (elements output) (\(val, count) -> 
  length (filter (==val) input) == count)
...
genList :: Gen [Int]
genList = do
  let nums = elements [0..10]
  lengths <- choose (3,5)
  result <- vectorOf lengths nums
  return $ sort result
...
instance Arbitrary Arg where
  arbitrary = do
    num <- elements [0..10]
    var <- elements (['a'..'c'] ++ ['x'..'z'])
    makeNum <- choose (True, False)
    if makeNum then return $ Number num
      else return $ Variable var
...
instance Arbitrary Expression where
  arbitrary = do
    ...
    makeNum <- choose (True, False)
    if makeNum then return $ Plus arg1 arg2
      else return $ Minus arg1 arg2
