...
member item (Set s) = item `elem` s

-- add a member to a set
add :: (Eq a, Ord a) => a -> Set a -> Set a
add item (Set s) = Set (sort . nub $ item:s)
...
data State = Start | Error | Finished | AddedEggs | AddedFlour | AddedSugar | AddedFlourAndSugar | Mixed
  deriving (Eq,Show)

step :: State -> Event -> State
step state event = case state of
  Start -> if event == AddEggs then AddedEggs else Error
  AddedEggs -> case event of
               AddFlour -> AddedFlour
               AddSugar -> AddedSugar
               _ -> Error
  AddedFlour -> if event == AddSugar then AddedFlourAndSugar else Error
  AddedSugar -> if event == AddFlour then AddedFlourAndSugar else Error
  AddedFlourAndSugar -> if event == Mix then Mixed else Error
  Mixed -> if event == Bake then Finished else Error
  Finished -> Finished
  Error -> Error
...
show1 (Add1 i j) = showNums i j "+"
show1 (Subtract1 i j) = showNums i j "-"
show1 (Multiply1 i j) = showNums i j "*"

showNums :: (Show a1, Show a2) => a1 -> a2 -> String -> [Char]
showNums i j op = show i ++ op ++ show j
...
instance Operation2 Add2 where
  ...
  show2 (Add2 i j) = showNums i j "+"

instance Operation2 Subtract2 where
  compute2 (Subtract2 i j) = i-j
  show2 (Subtract2 i j) = showNums i j "-"

instance Operation2 Multiply2 where
  compute2 (Multiply2 i j) = i*j
  show2 (Multiply2 i j) = showNums i j "*"
...
passwordAllowed p req = case req of
  MinimumLength l -> length p >= l
  ContainsSome chars -> containsSome chars
  DoesNotContain chars -> not $ containsSome chars
  And req1 req2 -> passwordAllowed p req1 && passwordAllowed p req2
  Or req1 req2 -> passwordAllowed p req1 || passwordAllowed p req2
  where containsSome s = foldr (\x prev -> (x `elem` p) || prev) False s
------------------------------------------------------------------------------
...
data Arithmetic = Literal Integer | Operation Op Arithmetic Arithmetic deriving Show

literal :: Integer -> Arithmetic
literal = Literal

operation :: String -> Arithmetic -> Arithmetic -> Arithmetic
operation opString arg1 arg2 = case opString of
  "+" -> Operation Add arg1 arg2
  "-" -> Operation Subtract arg1 arg2
  "*" -> Operation Multiply arg1 arg2
  _ -> Literal 0 -- should raise error

evaluate :: Arithmetic -> Integer
evaluate (Literal i) = i
evaluate (Operation op (Literal l1) (Literal l2)) = implementOp op l1 l2
evaluate (Operation op a1 a2) = implementOp op (evaluate a1) (evaluate a2)

implementOp :: Num a => Op -> a -> a -> a
implementOp op l1 l2 = case op of
  Add -> l1 + l2
  Subtract -> l1 - l2
  Multiply -> l1 * l2
...
render (Operation op (Literal i) (Literal j)) = "(" ++ show i ++ show op ++ show j ++ ")"
render (Operation op a1 a2) = "(" ++ render a1 ++ show op ++ render a2 ++ ")"
